...
let objToQs = obj => {
  return Object.keys(obj).reduce((before, value, index, keys) => {
    if (keys.length === index + 1) return `${before}${value}=${obj[value]}`;
    else return `${before}${value}=${obj[value]}&`;
  }, '');
};
...
// export async function reqChanegRole(params) {
//   return request('', {
//     method: 'POST',
//     data: params
//   })
