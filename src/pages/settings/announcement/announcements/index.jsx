...
class TableList extends Component {
  ...
  handleDelete = id => {
    ...
    dispatch({
      ...
      payload: {
        announcementId: id,
        role: getAuthority(localStorage.getItem('antd-pro-authority'))[0],
      },
    });
  };
  handleView = id => {
    ...
    this.props.history.push('/settings/announcement/detail');
  };
  handleModify = id => {
    const { dispatch } = this.props;
    dispatch({
    ...
    this.props.history.push('/settings/announcement/append');
  };
  editWarning = () => {
    Modal.warning({
      title: '提醒',
  ...
  componentDidMount() {
    const { dispatch } = this.props;
    // console.log(getAuthority(localStorage.getItem('antd-pro-authority'))[0])
    if (getAuthority(localStorage.getItem('antd-pro-authority'))[0] === 4) {
      dispatch({
        type: 'announcement/fetchAllList',
      });
    } else {
      dispatch({
    ...
    return (
      <Form layout="inline">
        ...
        >
          <Col md={8} sm={24}>
            <FormItem label="公告标题">
              {getFieldDecorator('title')(<Input placeholder="标题"></Input>)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="状态">
