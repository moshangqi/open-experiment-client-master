...
class TableList extends Component {
  ...
  render() {
      ...
      <div>
        {isModify ? (
          <span style={{ marginRight: 18 }}>
            <span> 状态: {detail.status === 1 ? '已发布 ' : '未发布 '}</span>
            <Badge status={detail.status === 1 ? 'processing' : 'default'} />
          </span>
        ) : (
          ''
        )}
        <Button type="primary" onClick={this.handlePublich}>
          发布
        </Button>
        {!isModify ? (
          ''
        ) : (
          <Button type="primary" style={{ marginLeft: 15 }} onClick={this.handleCancelPublish}>
            取消发布
          </Button>
        )}
        <Button style={{ margin: '0 15px' }} onClick={this.handleSave}>
          保存
        </Button>
        <Button
          onClick={() => {
    ...
    return (
      <PageHeaderWrapper extra={extra} title={isModify ? '修改公告' : '新增公告'}>
        <Card title={title}>
          正文
          <ReactQuill
