...
@connect(({ listTableList, loading,hitKeyproject }) => ({
  listTableList,
  loading: loading.models.hitKeyproject,
  projects:hitKeyproject.projects,
}))



class MidHitKeyProject extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'hitKeyproject/getKeyProjectMidTermReturnProject',
      payload:{
        data:{}
      }
    });
  ...
  handleModalOk = ()=>{
    ...
    if(approvalType === 0){
      dispatch({
        type:'hitKeyproject/rejectIntermediateInspectionKeyProject',
        payload:{
          data,
        }
      })
      ...
      dispatch({
        type:'hitKeyproject/KeyProjectMidTermReviewPassed',
        payload:{
          data,
        }
      })
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper>
        ...
        >
          <TextArea onChange={this.handleInputChange} style={{height:150}} value={text} placeholder={approvalType===0?'中期复核失败理由':'中期复核同意理由'}/>

        </Modal>
        <Card bordered={false} title='中期驳回项目审理'  style={{marginTop:15}}>
...
export default Form.create()(MidHitKeyProject);
