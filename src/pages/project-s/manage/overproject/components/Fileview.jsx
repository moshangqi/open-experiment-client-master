...
class Fileview extends Component {
  ...
  downloadApplyModel = ()=>{
    ...
    saveAs('http://zty200329.nat300.top/document/开放实验重点项目申请书正文参考模板.doc','开放实验重点项目申请书正文参考模板.doc')
  }
  showModal = (type)=>{
    this.setState({
      modalVisible:true,
  ...
  render() {
    ...
    const {detail,OverList,loading,budget={},membersInfo={}} = this.props
    ...
    const props = {
      beforeUpload: file => {
        ...
        dispatch({
          type:'detail/uploadConcludingReport',
          payload:{
            data:formData,
            id:detail.id,
            file
      ...
      <Spin tip="请耐心等候..." spinning={loading}>
        ...
        >
          <div style={{ width: '50%', float: 'left' }}>
            <Upload {...props} style={{ width: '200', float: 'left' }} fileList={OverList} showUploadList={{showRemoveIcon:false}}>
              <Button loading={loading}>
                <Icon type="upload" /> {detail.overurl?'重新上传':'上传'}
              </Button>
            </Upload>
          </div>
...
export default Form.create()(Fileview);
