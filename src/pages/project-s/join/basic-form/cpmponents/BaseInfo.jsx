...
export default ({detail})=>(
      <Descriptions  size="small" column={3} >
        ...
        <Descriptions.Item label="适应专业">{detail.limitMajor===null?'不限专业':JSON.parse(detail.limitMajor).map(item=>{
          return major.find(i=>i.mId===item).mName
        }).join('、')}</Descriptions.Item>
        <Descriptions.Item label="限选学院">{detail.limitCollege===null?'不限学院':JSON.parse(detail.limitCollege).map(item=>{
          return majorCollege[item-1].cName
        }).join('、')}</Descriptions.Item>
        <Descriptions.Item label="限选年级">{detail.limitGrade===null?'不限年级':JSON.parse(detail.limitGrade).map(item=>{
          return item+'级'
        }).join('、')}</Descriptions.Item>
