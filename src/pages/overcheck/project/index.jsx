...
@connect(({ listTableList, loading, over }) => ({
  listTableList,
  loading: loading.models.over,
  projects: over.projects,
}))
class OverCheck extends Component {
  state = {
    expandForm: false,
    formValues: {},
    selectedRows: [],
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'over/collegeGetsTheItemsToBeCompleted',
      payload: {
        data: {},
      },
    });
  ...
  stopProject = id => {
    const { dispatch } = this.props;
    let data = [{ projectId: id, reason: '学院结题审核不通过' }];
    console.log(data);
    dispatch({
      type: 'over/CollegeRejectToBeConcludingProject',
      payload: {
        data,
      },
    });
...
export default Form.create()(OverCheck);
